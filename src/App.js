      ...
      <div className="container">
        <div className="full-screen">
          ...
          <div className="main">
            ...
            { roll && roll.map((player) => {
              console.log('PLAYER: ', player)
              return <RollCard key={uuidv4()} player={player} units={units.filter((unit) => { return player.units.includes(unit.name)})} />
            })}
          </div>
          <div className="button-container">
            <Button theme="positive" label="roll" onClick={this.handleRoll} />
          </div>
        </div>
          </div>
